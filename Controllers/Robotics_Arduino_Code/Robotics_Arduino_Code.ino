...
void loop()                     
{
  ...
  if((otherRobot + noInput) < invalidWaitCount){      
    if(inByte != -1){
      if((inByte > (myBase -1)) && (inByte < (myBase + 30))){
       ...
       //save the old value for comparison in next loop
       old = inByte; 
      }
      else{
